...
export function downloadReportCSV(reports: DailyPerformanceReport[], employeeName: string) {
  ...
  const rows = reports.map(r => [
    ...
    `"${r.dayOfWeek || ''}"`,
    r.depositsETB || 0,
    r.foreignCurrencyETB || 0,
    r.digitalFinancialServicesETB || 0,
    r.accountOpenings || 0,
    r.mobileBankingActivations || 0,
    r.internetBankingActivations || 0,
    r.merchantSolutionsActivations || 0,
    r.atmCardsIssued || 0,
    `"${r.status || ''}"`,
...
export function downloadReportExcel(reports: DailyPerformanceReport[], employeeName: string, user?: User) {
  ...
  const totalFCY = reports.reduce((s, r) => s + (r.foreignCurrencyETB || 0), 0);
  const totalDFS = reports.reduce((s, r) => s + (r.digitalFinancialServicesETB || 0), 0);
  const totalAccounts = reports.reduce((s, r) => s + (r.accountOpenings || 0), 0);
  const totalMobile = reports.reduce((s, r) => s + (r.mobileBankingActivations || 0), 0);
  const totalInternet = reports.reduce((s, r) => s + (r.internetBankingActivations || 0), 0);
      ...
      <table>
        ...
        ${reports.map(r => `
          <tr>
            <td>${r.reportDate}</td>
            <td>${r.dayOfWeek}</td>
            <td class="num">${(r.depositsETB || 0).toLocaleString()}</td>
            <td class="num">${(r.foreignCurrencyETB || 0).toLocaleString()}</td>
            <td class="num">${(r.digitalFinancialServicesETB || 0).toLocaleString()}</td>
            <td class="num">${r.accountOpenings || 0}</td>
            <td class="num">${r.mobileBankingActivations || 0}</td>
            <td class="num">${r.internetBankingActivations || 0}</td>
            <td class="num">${r.merchantSolutionsActivations || 0}</td>
            <td class="num">${r.atmCardsIssued || 0}</td>
            <td>${r.status}</td>
        ...
        <tr class="total">
          ...
          <td class="num">${totalFCY.toLocaleString()}</td>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr class="total-row">
            ...
            <td>ETB ${reports.reduce((s, r) => s + (r.foreignCurrencyETB || 0), 0).toLocaleString()}</td>
