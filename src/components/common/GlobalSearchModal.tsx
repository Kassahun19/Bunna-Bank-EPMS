...
export const GlobalSearchModal: React.FC<GlobalSearchModalProps> = ({
  ...
  const matchedEmployees = q ? employees.filter(e => e.firstName.toLowerCase().includes(q) || (e.middleName && e.middleName.toLowerCase().includes(q)) || e.lastName.toLowerCase().includes(q) || e.jobTitle.toLowerCase().includes(q)) : [];
  const matchedBranches = q ? branches.filter(b => b.name.toLowerCase().includes(q) || b.code.toLowerCase().includes(q)) : [];
  const matchedDistricts = q ? districts.filter(d => d.name.toLowerCase().includes(q) || d.region.toLowerCase().includes(q)) : [];
  const matchedReports = q ? reports.filter(r => r.employeeName.toLowerCase().includes(q) || r.reportDate.includes(q) || r.status.toLowerCase().includes(q)) : [];
  const matchedKPIs = q ? kpis.filter(k => k.name.toLowerCase().includes(q) || k.code.toLowerCase().includes(q)) : [];
