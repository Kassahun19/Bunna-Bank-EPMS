...
export const EmployeePerformanceModal: React.FC<EmployeePerformanceModalProps> = ({
  ...
  const [activeViewTab, setActiveViewTab] = useState<'products' | 'submissions'>('products');
  ...
  const employeeReports = reports.filter(r => {
    ...
    const isMatch = r.employeeId === selectedEmployee.id || 
      (r.employeeName && r.employeeName.toLowerCase().includes(selectedEmployee.firstName.toLowerCase()));
  ...
  const productPerformance = PRODUCT_DEFINITIONS.map(prod => {
    ...
    const matchedTargetObj = targets.find(t =>
      ...
      (t.kpiId === prod.kpiId || t.kpiName.toLowerCase().includes(prod.name.toLowerCase()))
    );
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="p-4 rounded-2xl bg-[#0B4228] border border-white/10 space-y-1">
              ...
              <p className="text-[10px] text-gray-300">
                Target: ETB {(productPerformance.find(p => p.key === 'foreignCurrencyETB')?.target || 0).toLocaleString()}
              </p>
            </div>
